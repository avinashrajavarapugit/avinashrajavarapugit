...
| [Intrusion Detection System](https://github.com/avinashrajavarapugit/Intrusion-Detection-System-IDS-using-Signature-based-and-Anomaly-based-Approaches) | Network intrusion detection built two ways — signature matching and anomaly modelling — with separate training and evaluation pipelines for each, so the approaches can be compared directly. | Python, scikit-learn, Pandas |
| [Rule Engine with AST](https://github.com/avinashrajavarapugit/Rule-Engine-with-AST-and-Weather-Data-Monitoring) | Evaluates eligibility rules by parsing them into an abstract syntax tree, supporting rules created and combined at runtime rather than hardcoded. | JavaScript, Node.js |
